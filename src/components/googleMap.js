        ...
        <Modal isOpen={props.mapOpen} toggle={()=>props.setMapOpen(!props.mapOpen)} size='xl'>
            <ModalHeader toggle={()=>props.setMapOpen(!props.mapOpen)}>
                <div className='row align-items-center map_search_header'>
                    <div>
                        <Search setSearchAttr={setSearchAttr} panTo={panTo} setMarkers={setMarkers} setPlaceStatus={setPlaceStatus} setFinalPlace={setFinalPlace}/>
                    </div>
...
function Search({ panTo, setSearchAttr, setMarkers, setPlaceStatus, setFinalPlace }){
    ...
    return(
        <Combobox 
            ...
            <ComboboxInput style={{borderRadius:'18px', boxShadow:'0px 1px 1px', border:'0.01px'}}  className='search_combobox' value={value} onChange={(event)=>setValue(event.target.value)} disabled={!ready} placeholder='Seach Place' />
